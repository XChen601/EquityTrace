...
const PopularSection = ({addToFavorites}) => {
    ...
    const getTopStockSymbols = async (number) => {
        const tradierToken = process.env.REACT_APP_TRADIER_TOKEN     
        const response = await fetch(`https://api.tradier.com/v1/markets/lookup?q=`, {
            method: 'GET',
            headers: {
    ...
    const getTopStockInfo = async (number) => {
        ...
        console.log(symbolStringList)
        const response = await fetch(`https://api.tradier.com/v1/markets/quotes?symbols=${symbolStringList}&greeks=false`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${tradierToken}`,
                'Accept': 'application/json'
    ...
    <div className='popular-section'>
        ...
        <div className='card-section'>
            ...
            <div className='stock-card'>
                ...
                <button className="favorite-btn" onClick={() => addToFavorites(stock.symbol, stock.ask)}>Favorite!</button>
            </div>
            ))} 
        </div>
